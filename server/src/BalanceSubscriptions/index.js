...
const DAI_ALLOWANCE_UPDATED = "DAI_ALLOWANCE_UPDATED";
...
let handleAllowance = async function (src, guy, wad) {
  dai.allowance(src, guy).then((allowance) => {
    pubsub.publish(DAI_ALLOWANCE_UPDATED, {
      daiAllowance: {id: src, balance: allowance.toString()},
    });
  });
};
...
function _subscribe_allowance(id, contractID) {
  let filterAllowance = dai.filters.Approval(id, contractID);
  let filterTransfer = dai.filters.Transfer(id, contractID);

  dai.on(filterAllowance, handleAllowance);
  dai.on(filterTransfer, handleAllowance);

  dai.allowance(id, contractID).then((allowance) => {

    pubsub.publish(DAI_ALLOWANCE_UPDATED, {
      daiAllowance: {id, balance: allowance.toString()},
    });
  });
}

function _unsubscribe_alowance(id, contractID) {
  // Approvals
  let filterAllowance = dai.filters.Approval(id, contractID);
  let filterTransfer = dai.filters.Transfer(id, contractID);

  dai.off(filterAllowance, handleAllowance);
  dai.off(filterTransfer, handleAllowance);
}
