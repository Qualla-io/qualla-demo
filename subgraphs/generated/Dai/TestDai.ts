...
export class TestDai extends ethereum.SmartContract {
  static bind(address: Address): TestDai {
    return new TestDai("TestDai", address);
  }
  ...
  backendUser(): Address {
    let result = super.call("backendUser", "backendUser():(address)", []);

    return result[0].toAddress();
  }

  try_backendUser(): ethereum.CallResult<Address> {
    let result = super.tryCall("backendUser", "backendUser():(address)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
  ...
  try_subscriptionContract(): ethereum.CallResult<Address> {
    let result = super.tryCall(
      "subscriptionContract",
      "subscriptionContract():(address)",
      []
